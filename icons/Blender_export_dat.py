...
blender perspective.blend --background --python Blender_export_dat.py
...
def main():
    ...
    group = bpy.data.collections.get("Export")
    ...
    output_dat_dir = os.getcwd()


    for name, ob in objects:
        if ob.parent:
            continue
        # Use the already constructed output_dat_dir
        filename = os.path.join(output_dat_dir, name + ".dat")
